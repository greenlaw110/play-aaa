...
public class Enhancer extends play.classloading.enhancers.Enhancer {
   ...
   private void enhance_(ApplicationClass applicationClass, boolean buildAuthorityRegistryOnly) throws Exception {
      ...
      for (final CtMethod ctMethod : ctClass.getDeclaredMethods()) {
         ...
         IRight r = rr == null ? null : AnnotationHelper.getRequiredRight(rr, key);
         IPrivilege p = rp == null ? null : AnnotationHelper.getRequiredPrivilege(rp, key);
         
         if (r == null && p == null) {
            throw new RuntimeException("Invalid AAA annotation found: neither privilege nor right could be identified");
         }
         Authority.registAuthoriable(key, r, p);
         if (!buildAuthorityRegistryOnly) {
        	 ctMethod.insertBefore("play.modules.aaa.enhancer.Enhancer.Authority.checkPermission(\"" + key + "\", " + Boolean.toString(allowSystem) + ");");
             if (null != ra) {
                 String msg = ra.value();
                 if (null == msg || "".equals(msg)) msg = key;
                 ctMethod.insertBefore("play.modules.aaa.utils.Accounting.info(\"" + msg + "\", " + Boolean.toString(allowSystem) + ", $$);");
                 CtClass etype = ClassPool.getDefault().get("java.lang.Exception");
                 ctMethod.addCatch("{play.modules.aaa.utils.Accounting.error($e, \"" + msg + "\", " + Boolean.toString(allowSystem) + ", $$); throw $e;}", etype);
              }
         }
      }
      if (!buildAuthorityRegistryOnly) {
   ...
   public void buildAuthorityRegistry() throws Exception {
	   if (Authority.reg_.size() > 0) return; //suppose it's already built up during code enhancement
   ...
   public static class Authority implements IAuthorizeable {
      ...
      private Authority(IRight r, IPrivilege p) {
         if (null == r && null == p) {
            throw new IllegalArgumentException("at least one of right and privilege should be non-null value");
         }
         r_ = r;
         p_ = p;
      }
