...
public class Account extends GenericAccount {
   ...
   public static IAccount connect(String name, String password) {
      Account account = filter("_id", name).filter("password", passwordHash(name, password)).get();
      cur_.set(account);
      return account;
   }
   ...
   public static IAccount system() {
      if (null == sys_) {
         String sys = Play.configuration.getProperty(ConfigConstants.SYS_ACCOUNT, SYSTEM);
   ...
   protected static play.db.Model.Factory mf = MorphiaPlugin.MorphiaModelLoader.getFactory(Account.class);
   ...
   public List<IAAAObject> _fetch(int offset, int length, String orderBy,
       ...
       for (play.db.Model m: Account.getModelFactory().fetch(offset, length, orderBy, orderDirection, properties, keywords, where)) {
           l.add((Account)m);
       }
       return l;
   }
