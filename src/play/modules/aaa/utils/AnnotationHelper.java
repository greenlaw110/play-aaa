...
public class AnnotationHelper {
   ...
   public static IRight getRequiredRight(Method method) {
      return getRequiredRight(method.getAnnotation(RequireRight.class), method);
   }
    ...
    * 
    * @param clazz
    * @return required privilege or <code>null<code> if any exception encountered
    */
   public static IPrivilege getRequiredPrivilege(Class<?> clazz) {
      return getRequiredPrivilege(clazz.getAnnotation(RequirePrivilege.class), clazz);
   }
   ...
   public static IPrivilege getRequiredPrivilege(Method method) {
      return getRequiredPrivilege(method.getAnnotation(RequirePrivilege.class), method);
   }

   public static IPrivilege getRequiredPrivilege(RequirePrivilege rp, Object subject) {
      IPrivilege p = null;
      if (null != rp) {
         String s = rp.value();
         if (null != s && s.startsWith("aaa.")) {
         ...
         if (null != s) {
            try {
               ...
               Logger.warn(e, "Error loading IPrivilege implementation from %1$s", subject);            
            }
         }
      }
      if (null == p) {
